...
WEAPONS = {
    # --- Ближнее (9) ------------------------------------------------------
    "w_cleaver": dict(name="Ржавый тесак", g="m", cls=MELEE, tags=["blade", "primitive"],
                      damage=10, cooldown=0.90, range=105, knockback=6, crit=5,
                      scaling={"melee_dmg": 1.0, "damage_pct": 1.0},
                      shape=arc(80), price=12, open=True),
    "w_chainblade": dict(name="Цепной клинок", g="m", cls=MELEE, tags=["blade", "heavy"],
    ...
    "w_hammer": dict(name="Силовой молот", g="m", cls=MELEE, tags=["blunt", "heavy"],
    ...
    "w_censer": dict(name="Кадило-цеп", g="m", cls=MELEE, tags=["blunt", "holy"],
                     damage=9, cooldown=0.85, range=110, knockback=10, crit=3,
                     scaling={"melee_dmg": 1.0, "damage_pct": 1.0},
                     shape=arc(120), price=15, open=True),
    "w_scourge": dict(name="Шипастый бич", g="m", cls=MELEE, tags=["blade", "primitive"],
    ...
    # --- Дальнее (9) ------------------------------------------------------
    "w_nailer": dict(name="Гвоздомёт", g="m", cls=RANGED, tags=["gun", "primitive", "spread"],
    ...
    "w_spiker": dict(name="Шпилевик", g="m", cls=RANGED, tags=["gun", "precise"],
    ...
    "w_shotgun": dict(name="Обрез", g="m", cls=RANGED, tags=["gun", "spread", "heavy"],
                      ...
                      price=22, open=True),
    "w_autocannon": dict(name="Автопушка", g="f", cls=RANGED, tags=["gun", "heavy"],
    ...
    "w_carbine": dict(name="Лучевой карабин", g="m", cls=RANGED, tags=["gun", "precise"],
                      damage=11, cooldown=1.05, range=360, knockback=2, crit=8,
                      scaling={"ranged_dmg": 1.0, "damage_pct": 1.0},
                      shape=shot(speed=620, pierce=1, ttl=1.0, texture="p_beam"),
                      price=20, open=True),
    "w_longbarrel": dict(name="Длинноствол", g="m", cls=RANGED, tags=["gun", "precise", "heavy"],
    ...
    "w_lance": dict(name="Гарпунный арбалет", g="m", cls=RANGED, tags=["precise", "heavy"],
    ...
    "w_needler": dict(name="Игломёт", g="m", cls=RANGED, tags=["gun", "spread", "precise"],
    ...
    "w_grapeshot": dict(name="Картечница", g="f", cls=RANGED, tags=["gun", "spread", "heavy"],
    ...
    # --- Стихийное (7) ----------------------------------------------------
    "w_plasmacutter": dict(name="Плазменный резак", g="m", cls=ELEM, tags=["warp", "precise"],
                           ...
                           price=30, unlock_tier=2),
    "w_rod": dict(name="Жезл разлома", g="m", cls=ELEM, tags=["warp", "precise"],
                  ...
                  price=26, open=True),
    "w_venomsprayer": dict(name="Ядо-распылитель", g="m", cls=ELEM, tags=["warp", "spread"],
    ...
    "w_stormcaster": dict(name="Грозорассеиватель", g="m", cls=ELEM, tags=["warp", "spread"],
    ...
    "w_sporegun": dict(name="Споровая пушка", g="f", cls=ELEM, tags=["warp", "spread", "heavy"],
    ...
    "w_flamer": dict(name="Огнемёт", g="m", cls=ELEM, tags=["warp", "spread"],
    ...
    "w_icelens": dict(name="Ледяная линза", g="f", cls=ELEM, tags=["warp", "precise"],
    ...
    "w_turret": dict(name="Автотурель", g="f", cls=ENGI, tags=["construct", "gun"],
                     ...
                     price=24, open=True),
    "w_bonemine": dict(name="Костяная мина", g="f", cls=ENGI, tags=["construct", "heavy"],
    ...
    "w_forgedrone": dict(name="Литейный дрон", g="m", cls=ENGI, tags=["construct", "precise"],
...
def build(round_):
    ...
    for fam, b in WEAPONS.items():
        if "g" not in b:
            raise ValueError(f"семейство {fam}: не задан род/число (g)")
        if b["g"] not in TIER_PREFIX:
            raise ValueError(f"семейство {fam}: неизвестный род/число {b['g']!r}")
        prefixes = TIER_PREFIX[b["g"]]
        for tier in range(1, 5):
            name = b["name"] if tier == 1 else prefixes[tier - 1] + b["name"].lower()
